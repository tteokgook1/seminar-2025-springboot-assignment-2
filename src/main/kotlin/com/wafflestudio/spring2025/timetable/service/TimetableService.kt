...
class TimetableService(
    ...
    @Transactional
    fun createTimetable(user: User, request: TimetablesCreateRequest): TimetableDTO {
        if (timetableRepository.existsByUserIdAndNameAndYearAndSemester(
                user.id!!, request.name, request.year, request.semester
            )
        ) {
        ...
        @Transactional
        fun updateTimetable(user: User,id: Long, request: TimetablesUpdateRequest): TimetableDTO {
            val timetable = timetableRepository.findById(id)
            ...
            if (timetableRepository.existsByUserIdAndNameAndYearAndSemesterAndIdNot(
                    user.id!!, request.name, timetable.year, timetable.semester, id
                )
            ) {
        ...
        @Transactional
        fun deleteTimetable(user: User, id: Long) {
            val timetable = timetableRepository.findById(id)
                .orElseThrow { NotFoundException("해당 ID의 시간표를 찾을 수 없습니다.") }
