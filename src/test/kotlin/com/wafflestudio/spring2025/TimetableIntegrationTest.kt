...
class TimetableIntegrationTest
    ...
    constructor(
        ...
        fun `should retrieve timetable details`() {
            // 시간표 상세 정보를 조회할 수 있다
            val (user, token) = dataGenerator.generateUser()
            val timetable = dataGenerator.generateTimetable(user = user, year = 2025, semester = 1)
            val course = dataGenerator.generateCourse(2025, 1)
            val classTime = dataGenerator.generateClassTime(course, 0)
            val classTime2 = dataGenerator.generateClassTime(course, 2)
            ...
            val response =
                mvc
                    ...
                    .getContentAsString(Charsets.UTF_8)
                    .let {
                        mapper.readValue(it, TimetableDetailsDTO::class.java)
                    }
 ...
 */
        ...
        fun `should search for courses`() {
            // 강의를 검색할 수 있다
            repeat(5) {
                dataGenerator.generateCourse(2025, 1)
            ...
            mvc
                .perform(
                    get("/api/v1/courses?semester=1"),
                ).andExpect(status().isOk)

            val response =
                mvc
                    .perform(
                        get("/api/v1/courses?semester=3&q=${course.courseTitle}"),
                    ).andExpect(status().isOk)
                    .andReturn()
                    .response
                    .getContentAsString(Charsets.UTF_8)
        ...
        fun `should add a course to timetable`() {
            ...
            mvc
                ...
                .andExpect(jsonPath("$.credits").value(course.credits))
        }
        ...
        fun `should return error when adding overlapping course to timetable`() {
            ...
            val timetable = dataGenerator.generateTimetable(user = user, year = 2025, semester = 1)
            val course = dataGenerator.generateCourse(2025, 1)
            val course2 = dataGenerator.generateCourse(2025, 1)
            dataGenerator.generateClassTime(course, 0, 0, 1440)
            dataGenerator.generateClassTime(course2, 0, 0, 1440)
        ...
        @Test
        fun `should not add not-found course to timetable`() {
            // 존재하지 않는 강의를 시간표에 추가할 수 없다
            val (user, token) = dataGenerator.generateUser()
            val timetable = dataGenerator.generateTimetable(user = user, year = 2025, semester = 1)
        ...
        @Test
        fun `should not add a course to not-found timetable`() {
            // 강의를 존재하지 않는 시간표에 추가할 수 없다
            val (_, token) = dataGenerator.generateUser()
            val course = dataGenerator.generateCourse(2025, 1)

            mvc
                .perform(
                    post("/api/v1/timetables/123456789/courses")
                        .header("Authorization", "Bearer $token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(
                            mapper.writeValueAsString(
        ...
        @Test
        fun `should not add a course twice to timetable`() {
            // 시간표에 같은 강의를 여러 번 추가할 수 없다.
            val (user, token) = dataGenerator.generateUser()
            val timetable = dataGenerator.generateTimetable(user = user, year = 2025, semester = 1)
            val course = dataGenerator.generateCourse(2025, 1)
        ...
        @Test
        fun `should not add a course to timetable with different year or semester`() {
            // 학기가 다른 시간표에 강의를 추가할 수 없다.
            val (user, token) = dataGenerator.generateUser()
            val timetable = dataGenerator.generateTimetable(user = user, year = 2025, semester = 1)
            val course = dataGenerator.generateCourse(2024, 1)
            val course2 = dataGenerator.generateCourse(2025, 2)
        ...
        @Test
        fun `should not remove a course which is not in the timetable`() {
            // 시간표에 추가되지 않은 강의를 삭제할 수 없다
            val (user, token) = dataGenerator.generateUser()
            val timetable = dataGenerator.generateTimetable(user = user, year = 2025, semester = 1)
            val course = dataGenerator.generateCourse(2025, 1)
        ...
        @Test
        fun `should not remove a course from not-found timetable`() {
            // 없는 시간표에서 강의를 삭제할 수 없다
            val (_, token) = dataGenerator.generateUser()
            val course = dataGenerator.generateCourse(2025, 1)

            mvc
                .perform(
                    delete("/api/v1/timetables/123456789/courses/${course.id}")
                        .header("Authorization", "Bearer $token"),
                ).andExpect(status().isNotFound)
        }
        ...
        fun `should return correct course list and total credits when retrieving timetable details`() {
            ...
            mvc
                ...
                .andExpect(jsonPath("$.credits").value(courses.sumOf { it.credits }))
        }
        ...
        fun `should paginate correctly when searching for courses`() {
            ...
            val response =
                mvc
                    .perform(
                        get("/api/v1/courses?year=2025&semester=1&size=5"),
                    ).andExpect(status().isOk)
            ...
            mvc
                .perform(
                    get("/api/v1/courses?year=2025&semester=1&size=5&nextId=${response.meta.nextId}"),
                ).andExpect(status().isOk)
